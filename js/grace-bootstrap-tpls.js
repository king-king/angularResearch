        ...
        return {
            ...
            link: function ($scope, iElement, attrs, ctrls) {
                ...
                $scope.cancelSelect = function (value) {
                    var s = $scope.selection, config = $scope.config, result;
...
.directive('graceDaypicker', ['dateFilter', 'graceDateServ', function(dateFilter, graceDateServ) {
    return {
        ...
        link: function(scope, element, attrs, ctrl) {
            ...
            var DAYS_IN_MONTH = [31, 28, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31], hours, minutes;
            ...
            scope.hourValidate = function($event) {
                var target = $event.target, hour, date, nHour, $target = angular.element(target);
                var date = angular.copy(ctrl.activeDate), dt = scope.dt;
            ...
            scope.minuteValidate = function($event) {
                var target = $event.target, $target = angular.element(target);
                var minute = angular.element(target).val(), nMinute, dt = scope.dt;
...
//                $scope.enableTooltip = angular.isDefined(data.enableTooltip) ? data.enableTooltip : gridConstant.enableTooltip;
            ...
            self.setScrollerWidth = function(){
                var el=$scope.gridContainerEl[0], wrapper=el.querySelector('.grace-grid-wrapper'),
                     horizonScroller = el.querySelector('.normal-scroller'), horizonScrollerInner = wrapper.querySelector('.normal-scroller-inner');
                
                if(!horizonScroller){
                    return ;
                }

                var  pinningHeader=el.querySelector('.pinning-header'), pinningHedaerWidth=pinningHeader?pinningHeader.offsetWidth:0;
                var  normalHeader = el.querySelector('.normal-header'), normalHeaderWidth = normalHeader?normalHeader.offsetWidth:0;
...
//                            selectallTip.css(cssObj);
                        ...
                        function lockHeader2Top(event){
                            ...
                            var topLockHeight = topLockOffsetTop = 0, offsetTop;
...
//            minWidth:gridConstant.minColumnWidth,
        ...
        function calAutoColumnWidth(minWidthColumns, autoWidthColumnsLength, totalUnmallocWidth){
            var stop = 0;
            for(var i=0, ih;(ih=minWidthColumns[i]); i++){
        ...
        function autoColumnWidth(headerInfo){
            ...
            if(!headerInfo.autoWidthColumns.length) return;
            var isPined = headerInfo.columns.filter(function(a){return a.pinned}).length>0;
            var broType = gridUtil.browserType;
...
//            var autoTotalWidth = getTotalWidth(headerInfo.autoWidthColumns.filter(function(a){return a.visible}));
            ...
            var autoWidthColumns = headerInfo.autoWidthColumns.filter(function(a){return a.visible});
            var minWidthColumns =  headerInfo.autoWidthColumns.filter(function(a){return a.minWidth});
    ...
    .directive("graceSummary", ["$timeout", "$window", "summaryConfig", function ($timeout, $window, summaryConfig) {
     ...
     .filter("summaryCal",["$filter",function($filter){
        ...
        return function(input,param, direct){
            var tmp = param.split('_'), isShow = Number(tmp[3]), input = Number(input) ;
