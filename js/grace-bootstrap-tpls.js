            ...
            link: function($scope, iElement, attrs, ctrls) {
                ...
                $scope.cancelSelect = function(value) {
                    var s = $scope.selection,
                        config = $scope.config,
                        result;
                    if (config.callback && config.callback.beforeCancelSelect) {
                        result = config.callback.beforeCancelSelect(value, angular.copy($scope.selection)) === false;
                        if (result) {
                            return;
                        }
                    }
...
.constant("chosenAssociateConfig", {
    ...
    .controller("chosenAssociateController", ["$scope", "$http", "chosenAssociateConfig", "$sce", "$timeout", function($scope, $http, chosenAssociateConfig, $sce, $timeout) {
        ...
        $scope.choiceCloseClick = function(con, evt) {
            evt.stopPropagation();
            for (var i = 0, idata;
                (idata = $scope.ngModel['selected'][i]); i++) {
...
.directive("graceChosenAssociate", ["$window", function(win) {
    ...
    return {
        ...
        link: function(scope, element, attr, ctrl) {
            ...
            scope.getValue = function() {
                var select = scope.ngModel['selected'],
                    value;
                if (scope.plural) {
                    value = select.map(function(item, index, list) {
                        return item.code; });
                } else {
                    value = select.code || '';
                }
        ...
        */
        var Handle = {
                ...
                modelString: function(model) {
                    ...
                    } else if (/^#parentKey$/.test(model.replace(/{{(.*?)}}/g, function($1, $2) {
                            if ($2) {
                                return $2 } else {
                                return $1 } }))) {
...
Date.prototype.toJSON = function() {
        return this.toString(); } //JSON化Date时默认转换为当前时区
angular.module("grace.bootstrap.date", [])
    ...
    .factory('graceDateServ', ["graceDateConfig", function(_graceDateConfig) {
        ...
        var daysOfPreviousWeek = function(_date, _n, _pattern) {
            ...
            _pattern = _pattern || _graceDateConfig.pattern_ymd;
            _n = _n || 1;
            var day = previousDay(_date, 7 * _n, _pattern).date,
                days = daysOfCurrentWeek(day, _pattern);
            return days;
        };
        /**
         * 获取本月第N天的日期
    ...
    .filter("graceDateFilter", ['graceDateServ', function(_graceDateServ) {
        ...
        return function(_input, _options) {
            ...
            switch (options.type) {
                /* 本周的开始、结束日期 START */
                case 'current_week_121':
                    days = _graceDateServ.daysOfCurrentWeek(input, options.pattern);
                    returnDates = [days[0].format, days[days.length - 1].format];
                    break;
                    /* 本周的开始、结束日期 END */
                    /* 上N周的开始、结束日期 START */
                case 'previous_week_121':
                    days = _graceDateServ.daysOfPreviousWeek(input, options.n || 1, options.pattern);
                    returnDates = [days[0].format, days[days.length - 1].format];
                    break;
                    /* 上N周的开始、结束日期 END */
                    /* 本月的开始、结束日期 START */
                case 'current_month_121':
                    days = _graceDateServ.daysOfCurrentMonth(input, options.pattern);
                    returnDates = [days[0].format, days[days.length - 1].format];
                    break;
                    /* 本月的开始、结束日期 END */
                    /* 上N月的开始、结束日期 START */
                case 'previous_month_121':
                    days = _graceDateServ.daysOfPreviousMonth(input, options.n || 1, 0, options.pattern);
                    returnDates = [days[0].format, days[days.length - 1].format];
                    break;
                    /* 上N月的开始、结束日期 END */
                    /* 本季度的开始、结束日期 START */
                case 'current_season_121':
                    days = _graceDateServ.daysOfCurrentSeason(input, options.pattern);
                    returnDates = [days[0].format, days[days.length - 1].format];
                    break;
                    /* 本季度的开始、结束日期 END */
                    /* 上季度的开始、结束日期 START */
                case 'previous_season_121':
                    days = _graceDateServ.daysOfPreviousSeason(input, options.pattern);
                    returnDates = [days[0].format, days[days.length - 1].format];
                    break;
                    /* 上季度的开始、结束日期 END */
                    /* 默认当前日期 START */
                default:
                    returnDates = [_graceDateServ.format(input, options.pattern)];
                    break;
                    /* 默认当前日期 END */
            }
...
.controller('datepickerController', ['$scope', '$attrs', '$parse', '$interpolate', '$timeout', '$log', '$locale', 'dateFilter', 'datepickerConfig', 'graceDateServ', function($scope, $attrs, $parse, $interpolate, $timeout, $log, $locale, dateFilter, datepickerConfig, graceDateServ) {
    ...
    angular.forEach(['formatMinute', 'formatHour', 'formatDay', 'formatMonth', 'formatYear', 'formatDayHeader', 'formatMinuteTitle', 'formatHourTitle', 'formatDayTitle', 'formatMonthTitle',
        'minMode', 'maxMode', 'showWeeks', 'startingDay', 'graceYearRange', 'weekMode', 'showTimeSet', 'marker', 'showOutOfDayRange'
    ], function(key, index) {
    ...
    this.createDateObject = function(date, format) {
        ...
        if (!self.weekMode) {
            ...
            var modelWeek = self.getWeekNumber(model),
                curWeek = self.getWeekNumber(new Date()),
                week = self.getWeekNumber(date),
                hoverWeek = self.getWeekNumber($scope.hoverDate);
            return {
                date: date,
                label: format == 'MMMM' ? getMonthCN(parseInt(dateFilter(date, 'M'))) : dateFilter(date, format),
    ...
    this.createWeekObject = function(_weekObj) {
        ...
        return angular.extend({}, _weekObj, {
            ...
            hovered: _weekObj.week == graceDateServ.getISO8601WeekNumber($scope.hoverDate) && self.weekMode,
        })
    };
    ...
    this.tmpMove = function(direction) {
    ...
    $scope.mouseover = function(date, _dateStr) {
        if (self.weekMode) { //如果是周视图
            if (angular.isArray(date)) {
                if (_dateStr && /^\d{4}-\d{2}$/.test(_dateStr)) {
                    ...
                    // }
                    $scope.hoverDate = date[0];
                    self.refreshView();
                }
                return;
            } else {
                if ($scope.datepickerMode === self.minMode) {
                    $scope.hoverDate = date;
                    self.refreshView();
                }
            }
        }
    }
    $scope.mouseout = function(date, _dateStr) {
            if (self.weekMode) { //如果是周视图
                if (angular.isArray(date)) {
                    if (_dateStr && /^\d{4}-\d{2}$/.test(_dateStr)) {
                        // var year = _dateStr.split('-')[0],
    ...
    $scope.$on('graceDatepicker.marker', function() {
        //因为datepicker是手动编译的，只能在监听到marker发生变化后，从父scope中再取一次。
        self.marker = $scope.$eval($scope.$parent.marker);
        //marker 发生变更，重绘日历面板
        self.refreshView();
    });
...
.directive('graceDaypicker', ['dateFilter', 'graceDateServ', function(dateFilter, graceDateServ) {
    return {
        ...
        link: function(scope, element, attrs, ctrl) {
            ...
            scope.showTimeSet = ctrl.showTimeSet;
            scope.dayMarker = (!!ctrl.marker && !!ctrl.marker.day) ? ctrl.marker.day : [];

            scope.dt = {};
            // scope.showTimeSet = true;
            ctrl.step = { months: 1 };
            ...
            var DAYS_IN_MONTH = [31, 28, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31],
                hours, minutes;
            hours = ctrl.activeDate.getHours();
            minutes = ctrl.activeDate.getMinutes();

            scope.dt.hour = calLen(hours);
            scope.dt.minute = calLen(minutes);
            ...
            function getMarker(day) {
                var day = dateFilter(day, 'yyyy-MM-dd');
                var dayMarker;
                var classes = "";

                for (var i = 0; ctrl.marker && (dayMarker = ctrl.marker.day[i]); i++) {
                    if (dayMarker.days.length > 0 && (dayMarker.days.indexOf(day) !== -1)) {
                        classes += (" " + dayMarker.className);
                    }
                }
                return classes;
            }

            function getViewDates(activeDate) {
                ...
                // 按照智商的交互需求，改成35天一个月， by pengjuxiang 2016-8-30
            ...
            ctrl._refreshView = function() {
                ...
                for (var i = 0; i < 35; i++) {
                    ...
                    // days[i].updateTime = Math.random();
                    days[i].specialClass = getMarker(days[i].date);

                }

                var leftViewDates = getViewDates(ctrl.tmpMove(-1));
                var rightViewDates = getViewDates(ctrl.tmpMove(1));
                var allLeftViewDatesDisable = leftViewDates.every(function(date) {
                    return ctrl.isDisabled(date);
                });
                var allRightViewDatesDisable = rightViewDates.every(function(date) {
                    return ctrl.isDisabled(date);
                });
                scope.canNotMoveLeft = !ctrl.showOutOfDayRange && allLeftViewDatesDisable;
                scope.canNotMoveRight = !ctrl.showOutOfDayRange && allRightViewDatesDisable;
                ...
                scope.dt.hour = hours;
                scope.dt.minute = minutes;



            };
            scope.hourValidate = function($event) {
                var target = $event.target,
                    hour, date, nHour, $target = angular.element(target);
                var date = angular.copy(ctrl.activeDate),
                    dt = scope.dt;
                hour = angular.element(target).val();
                ...
                nHour = Number(hour);
                nHour > 23 ? (hour = '23') : (nHour < 0 ? (hour = '00') : '');

                //判断输入的日期是否大于最大日期
                date.setHours(Number(hour));
                //如果配置日期大于最大日期，则设置最大日期
                if (date > ctrl.maxDate) {
                    hour = ctrl.maxDate.getHours();
                }
                if (date < ctrl.minDate) {
                    hour = ctrl.minDate.getHours();
                }
                $target.val(hour);
                dt.hour = hour;
            ...
            scope.minuteValidate = function($event) {
                var target = $event.target,
                    $target = angular.element(target);
                var minute = angular.element(target).val(),
                    nMinute, dt = scope.dt;
                var date = angular.copy(ctrl.activeDate);
                ...
                nMinute = Number(minute);
                nMinute > 59 ? (minute = '59') : (nMinute < 0 ? (minute = '00') : '');

                //判断输入的日期是否大于最大日期
                date.setMinutes(Number(minute));
                //如果配置日期大于最大日期，则设置最大日期
                if (date > ctrl.maxDate) {
                    minute = ctrl.maxDate.getMinutes();
                }
                if (date < ctrl.minDate) {
                    minute = ctrl.minDate.getMinutes();
                }
            ...
            //实现选周mouse hove效果
            scope.hoverWeek = function(_weekObj) {
                    scope.mouseover(graceDateServ.getDatesByWeek(_weekObj.week, _weekObj.year), _weekObj.year + '-' + (_weekObj.week < 10 ? ('0' + _weekObj.week) : _weekObj.week));
                }
                //实现选周mouse hove效果
            scope.mouseoutWeek = function(_weekObj) {
                scope.mouseout(graceDateServ.getDatesByWeek(_weekObj.week, _weekObj.year), _weekObj.year + '-' + (_weekObj.week < 10 ? ('0' + _weekObj.week) : _weekObj.week));
            }
            ctrl.getWeekNumber = graceDateServ.getISO8601WeekNumber;

            ctrl.handleKeyDown = function(key, evt) {
                var date = ctrl.activeDate.getDate();
                var dateTmp = new Date(ctrl.activeDate);
                ...
                if (!ctrl.showOutOfDayRange) {
                    dateTmp.setMonth(date);
                    var nextViewDates = getViewDates(dateTmp);
                    var allViewDatesDisable = nextViewDates.every(function(date) {
                        return ctrl.isDisabled(date);
                    });
                    if (allViewDatesDisable) {
                        return;
                    }
                }
...
.directive('graceMonthpicker', ['dateFilter', 'graceDateServ', function(dateFilter, graceDateServ) {
    return {
        restrict: 'EA',
        replace: true,
        ...
        return {
            ...
            link: function(scope, element, attrs, ngModel) {
                var dateFormat,
                    ...
                    alignRight = angular.isDefined(attrs.alignRight) ? scope.$parent.$eval(attrs.alignRight) : datepickerPopupConfig.alignRight,
                    // showTimeSet  = angular.isDefined(attrs.alignRight) ? scope.$parent.$eval(attrs.showTimeSet) : datepickerPopupConfig.showTimeSet,
                ...
                scope.showOutOfDayRange = angular.isDefined(attrs.showOutOfDayRange) ? scope.$parent.$eval(attrs.showOutOfDayRange) : datepickerPopupConfig.showOutOfDayRange;
                ...
                // datepicker element
                // 传入options配置到 datepicker 指令
                var datepickerEl = angular.element(popupEl.children()[0]);
                if (attrs.datepickerOptions) {
                    angular.forEach(scope.$parent.$eval(attrs.datepickerOptions), function(value, option) {
                        //marker是对象(引用)，不能通过属性的方式传入，在grace-datepicker里，通过parent.parent的方式查找
                        if (option == 'marker' && !!value) {
                            datepickerEl.attr(cameltoDash(option), JSON.stringify(value));
                        } else {
                ...
                datepickerEl.attr(cameltoDash('showOutOfDayRange'), scope.showOutOfDayRange);

                scope.watchData = {};
                // 传入datepickerpopup的属性配置传到 datepicker 指令
                angular.forEach(['minDate', 'maxDate', 'datepickerMode', 'switchDisabled', 'showTimeSet', 'marker'], function(key) {
                    if (attrs[key]) {
                ...
                ngModel.$render = function() {
                    //angularjs会自动转换非空 $viewValue  为字符串
                    var viewValue = (typeof ngModel.$viewValue === 'string') ? ngModel.$viewValue.split(',') : [];
                    ...
                    if (angular.isArray(ngModel.$viewValue)) {
                        ...
                        angular.forEach(ngModel.$viewValue, function(val, i) {
                            // fix dateString yuhongping@jd.com 20160603
                            if (angular.isString(val)) val = new Date(val);
                        ...
                        if (!dateFormat || dateFormat.indexOf("w") == -1) {
                            // fix dateString yuhongping@jd.com 20160603
                            if (angular.isString(ngModel.$viewValue)) ngModel.$viewValue = new Date(ngModel.$viewValue);
                    ...
                    if (value) {
                        ...
                        $timeout(function() {
                            scope.$apply(function() {
                                ...
                                if (alignRight && !isNaN(alignRight)) {
                                    scope.position.left = scope.position.left + scope.position.width - pickerWidth + Number(alignRight);
                                }
                ...
                scope.$watch('datepickerOptions.marker', function(value, ovalue) {
                    if (value != ovalue && popupEl.children()) {
                        var datepickerEl = angular.element(popupEl.children().children()[0])
                        scope.marker = JSON.stringify(value);
                        scope.$broadcast('graceDatepicker.marker');
                    }
                });
...
.controller('datepickerRangeController', ['$scope', '$attrs', '$parse', '$interpolate', '$timeout', '$log', 'dateFilter', 'datepickerConfig', 'daterangepickerConfig', '$locale', function($scope, $attrs, $parse, $interpolate, $timeout, $log, dateFilter, datepickerConfig, daterangepickerConfig, $locale) {
    var self = this,
        ngModelCtrl = { $setViewValue: angular.noop }; // nullModelCtrl;
    /* 覆写周的显示 START */
    angular.extend($locale.DATETIME_FORMATS, {
    ...
    angular.forEach(['formatDay', 'formatMonth', 'formatYear', 'formatDayHeader', 'formatDayTitle', 'formatMonthTitle',
        'minMode', 'maxMode', 'showWeeks', 'startingDay', 'graceYearRange', 'pickerCount'
    ], function(key, index) {
...
.factory("dimensionFactory", ["dimensionConfig", function(dimensionConfig) {
    ...
    function createNodeIndex(nodeData, $scope) {
        var nodeIndex = {}!_scope && (_scope = $scope);
        if (!dimensionConfig.isCodeUniq) {
    ...
    .directive('graceDimensionBaseSelect', function() {
        return {
            ...
            link: function($scope, $element, $attr, $ctrls) {
                var dimensionBaseSelectCtrl = $ctrls[0],
                    ngModelCtrl = $ctrls[1];
                ...
                compile: function(tElem, tAttrs) {
                    ...
                    return function link(scope, element, attrs) {
                        ...
                        function hideTooltipBind() {
                            scope.$apply(function() {
                                $timeout(hide, scope.tt_popupDelay);
                            });
                        }
                        ...
                        function hide() {
                            ...
                            if (tooltip.del) {
                                if (scope.tt_animation) {
                                    if (!transitionTimeout) {
                                        transitionTimeout = $timeout(removeTooltip, 5);
                                    }
                                } else {
                                    removeTooltip();
                                }
                        ...
                        function createTooltip() {
                            ...
                            tooltip.on('mouseenter', function() {
                                tooltip.del = false;
                                tooltip.addClass("inout");
                            });
                            tooltip.on('mouseleave', function() {
                                $timeout(removeTooltip, scope.tt_popupDelay);
                            });
...
.controller("gridController", ["$scope", '$document', 'gridConstant', 'gridHeaderService', 'gridBodyService', 'gridUtil', '$timeout', 'GridApi', "$http",
    function($scope, $document, gridConstant, headerService, bodyService, gridUtil, $timeout, GridApi, $http) {
        ...
        //chrome 的时候不需要加补偿宽度。 在ff下， table会比tbody宽1个像素。
        // $scope.compensation =  broType.ie && 2 || broType.safari && 1 || broType.firefox && 2 || 0;
        ...
        $scope.valueParse = function(val, unit) {
            val += "";
            if (val == 'auto' || val.indexOf("%") != -1 || val.indexOf('px') != -1) return val;
        ...
        self.update = function(data) {
            ...
            //排序的顺序是否使用默认
            $scope.enableSortFirst = angular.isDefined(data.enableSort) && angular.isDefined(data.enableSortFirst) ? data.enableSortFirst : gridConstant.enableSortFirst;
            ...
            $scope.lockHeaderConfig = angular.isDefined(data.lockHeaderConfig) ? data.lockHeaderConfig : gridConstant.lockHeaderConfig;
        ...
        self.setScrollerWidth = function() {
            var el = $scope.gridContainerEl[0],
                wrapper = el.querySelector('.grace-grid-wrapper'),
                horizonScroller = el.querySelector('.normal-scroller'),
                horizonScrollerInner = wrapper.querySelector('.normal-scroller-inner');

            if (!horizonScroller) {
                return;
            }

            var pinningHeader = el.querySelector('.pinning-header'),
                pinningHedaerWidth = pinningHeader ? pinningHeader.offsetWidth : 0;
            var normalHeader = el.querySelector('.normal-header'),
                normalHeaderWidth = normalHeader ? normalHeader.offsetWidth : 0;


            var normalScrollerWidth = $scope.getViewPortInfo().width - pinningHedaerWidth;
            var normalScrollerInnerWidth = el.querySelector('.normal-header').offsetWidth;

            if (normalScrollerInnerWidth > normalScrollerWidth) {
                //inner width  normal width
                angular.element(horizonScroller).css('width', normalScrollerWidth + 'px').css('margin-left', pinningHedaerWidth + 'px').css('overflow-x', 'auto');
                angular.element(horizonScrollerInner).css('width', normalScrollerInnerWidth + 'px');
                angular.element(wrapper).css('overflow-x', 'hidden');

                horizonScroller.removeEventListener('scroll', self.scrollNormalBody);
                horizonScroller.addEventListener('scroll', self.scrollNormalBody);
            } else {
                angular.element(horizonScroller).css('overflow-x', 'hidden');
            }

        }

        self.scrollNormalBody = function(event) {
            var el = $scope.gridContainerEl[0];
            var scrollLeft = event.target.scrollLeft;
            var normalHeader = angular.element(el.querySelector(".normal-header"));
            var normalBody = angular.element(el.querySelector(".normal-body"));

            var cssObj = gridUtil.cssFixer("transform", "translateX(-" + scrollLeft + "px)");

            normalHeader.css(cssObj);
            normalBody.css(cssObj);
        }
            ...
            return {
                pre: function($scope, $elm, $attrs) {
                    ...
                    function getViewPortInfo() {
                        ...
                        width = width + Number(styles.borderRightWidth.replace('px', '')) + Number(styles.borderLeftWidth.replace('px', ''));
                ...
                post: function($scope, $elm, $attrs, ctrls) {
                    ...
                    function scrollNormalBody(event) {
                        var scrollLeft = event.target.scrollLeft;
                        var normalHeader = angular.element(wrapper.querySelector(".normal-header"));
                        var normalBody = angular.element(wrapper.querySelector(".normal-body"));
                    ...
                    var bakTopLockOffsetTop = ($scope.lockHeaderConfig && $scope.lockHeaderConfig.firstLockerOffsetTop) || 0;

                    function lockHeader2Top(event) {

                        var topLockHeight = topLockOffsetTop = 0,
                            offsetTop;

                        var scrollY = $scope.IE ? document.documentElement.scrollTop : $window.scrollY;
                        if ($scope.lockHeaderConfig) {
                            topLockOffsetTop = $scope.lockHeaderConfig.firstLockerOffsetTop || 0;
                            topLockHeight = $scope.lockHeaderConfig.firstLockerHeight || 0;
                        }

                        if (!headerOffset || bakTopLockOffsetTop != topLockOffsetTop) {
                            headerOffset = pos.offset(eheader);
                            bakTopLockOffsetTop = topLockOffsetTop;
                        }
                        offsetTop = headerOffset.top;

                        //如果第一层已开始锁定，且第一层遮挡住了表格，且表格没有开始锁定
                        if (scrollY > topLockOffsetTop && (offsetTop - scrollY) < topLockHeight && (offsetTop > scrollY)) {
                            eheader.css(gridUtil.cssFixer('transform', 'translateY(' + (topLockHeight - (offsetTop - scrollY)) + 'px)'));
                        } else if (offsetTop < scrollY) {
                            eheader.css(gridUtil.cssFixer('transform', 'translateY(' + (scrollY - offsetTop + topLockHeight) + 'px)'));
                        } else {
...
angular.module('grace.bootstrap.grid')
    .factory("GridApi", function() {
        ...
        GridApi.update = function(gridApi, ctrl) {
            ...
            gridApi.getRowDataRowIndex = function(field, val) {
                var bodyInfo = scope.bodyInfo;
                var rows = bodyInfo.rows;
                if (arguments.length == 1) {
                    val = field;
                ...
                for (var i = 0, ir;
                    ...
                    var rd = ir.rowInfo.rawData;
                    if (rd[field] === val) {
                        return ir.rowInfo.originalRowIndex;
                    }
                }
            }
...
.factory("gridBodyService", ['$gDataFormatterFactory', 'gridUtil', 'gridRowFactory', 'gridConstant', function(valueFormatter, gridUtil, rowFactory, gridConstant) {
    ...
    service.copyBody = function(srcBody) {
        ...
        for (var i = 0, ilen = srcBody.length; i < ilen; i++) {
            ...
            var tmpRow = srcBody[i],
                row = rowFactory.copy(tmpRow);
...
.controller("graceGridBodySimpleController", ['$scope', 'gridUtil', function($scope, gridUtil) {
    ...
    function getCellData(cellData, rowIndex) {
        ...
        if (cellData.type == 'buttons') {
            var content = '';
            for (var i = 0, ib;
                (ib = cellData.displayContent[i]); i++) {
                ib.text = ib.text || "";
...
.factory("gridHeaderService", ['gridConstant', 'gridUtil', function(gridConstant, gridUtil) {
    ...
    service.createHeader = function createHeader(head, $scope) {
        var headerInfo = {
            ...
            enableSortFirst: $scope.enableSortFirst,
            columns: [],
            columnsMap: {},
            autoWidthColumns: [],
            givenWidthColumns: [],
    ...
    function calAutoColumnWidth(minWidthColumns, autoWidthColumnsLength, totalUnmallocWidth) {
        var stop = 0;
        for (var i = 0, ih;
            (ih = minWidthColumns[i]); i++) {
            if (totalUnmallocWidth < ih.minWidth) {
                totalUnmallocWidth = (totalUnmallocWidth * autoWidthColumnsLength - ih.minWidth) / (--autoWidthColumnsLength);
                minWidthColumns.splice(i, 1);
                stop = 1;
                break;
            }
        }
        if (minWidthColumns.length > 0 && stop) {
            totalUnmallocWidth = calAutoColumnWidth(minWidthColumns, autoWidthColumnsLength, totalUnmallocWidth);
        }
        return totalUnmallocWidth
    }

    function autoColumnWidth(headerInfo) {

        if (!headerInfo.autoWidthColumns.length) return;
        var isPined = headerInfo.columns.filter(function(a) {
            return a.pinned }).length > 0;
        var broType = gridUtil.browserType;

        //talbe 设置border-collapse:collapse后，会比body宽1像素，所以将body设置小1像素，避免table宽度撑出滚动条 pengjuxiang 2016-12-9
        //firefox 下 normal-header 和 pinning-header 的table都会多一个宽度。 chrome只会多一个
        var abnormalWidth = 1;
        abnormalWidth += (broType.ie && isPined && 1 || broType.firefox && isPined && 1 || 0)

        var givenTotalWidth = getTotalWidth(headerInfo.givenWidthColumns.filter(function(a) {
            return a.visible }));
        //            var autoTotalWidth = getTotalWidth(headerInfo.autoWidthColumns.filter(function(a){return a.visible}));

        var autoWidthColumns = headerInfo.autoWidthColumns.filter(function(a) {
            return a.visible });
        var minWidthColumns = headerInfo.autoWidthColumns.filter(function(a) {
            return a.minWidth });

        var columnAutoWidth = gridConstant.minColumnWidth;

        //计算剩余宽度， 取出带最小宽度的列，  如果均分后的剩余宽度，小于任意最小宽度列，
        // 则剩余宽度减去最小宽度，继续均分
        totalUnmallocWidth = (headerInfo.viewPortWidth - givenTotalWidth - abnormalWidth) / autoWidthColumns.length;
        var autoWidthColumnsLength = autoWidthColumns.length;
        var totalUnmallocWidth = calAutoColumnWidth(minWidthColumns, autoWidthColumnsLength, totalUnmallocWidth)



        columnAutoWidth = Math.max(totalUnmallocWidth, columnAutoWidth);

        columnAutoWidth = Math.floor(columnAutoWidth);
        var autoColumnsTotalWidth = 0;
        for (var i = 0, ih;
            (ih = autoWidthColumns[i]); i++) {
        ...
        //把减少的宽度加回来  pengjuxiang 2016-12-9
        headerInfo.totalWidth += abnormalWidth;
    ...
    function fixColumn(columns, columnInfo) {
        ...
        for (var i = 0, ic;
            (ic = columnInfo.columnsByDepth[i]); i++) {
            for (var j = 0, jc;
                (jc = ic[j]); j++) {
                ...
                return {
                    post: function($scope, $elm, $attrs, ctrls) {
                        ...
                        gridCtrl.pinning = function(scope) {
                            ...
                            for (var i = 0, ib;
                                (ib = bodyInfo.rows2Render[i]); i++) {

                                var tmp = [];

                                for (var j = 0, jp;
                                    (jp = $scope.pinnings[j]); j++) {
                                    tmp.push(ib.splice(jp.columnIndex, 1, null)[0]);
                                }
                                tmp.rowInfo = ib.rowInfo;

                                ib = ib.filter(function(a) {
                                    return a != null });
            ...
            compile: function() {
                return {
                    pre: function($scope, $elm, $attrs, ctrls) {
                        ...
                        if (gridScope.tooltipConfig.position == "before" || !gridScope.tooltipConfig.position) {
                            html += tooltipTpl;
                        }
                        ...
                        if (cellData.columnType == 'rowSelector') {
                            html += '<span class="cell-value"  >' +
                                ...
                                '</span>';
                            if (gridScope.tooltipConfig.position == 'after') {
                                html += tooltipTpl;
                            }
                            html += '</div>';
                        } else {
                            html += '<span class="cell-value" ng-click="valueClick($event)"  >' +
                                ...
                                ((gridScope.tooltipConfig.position == 'after') ? tooltipTpl : '') +
                    ...
                    post: function($scope, $elm, $attrs, ctrls) {
                        ...
                        $scope.valueClick = function($event, typeIndex) {
                            ...
                            if (typeIndex != undefined) {
                                ...
                                // 更改默认排序为降序 yuhp 20161216
                                sortTypesIndex += 2;
                                sortTypesIndex %= 3;
                            }

                            if (!gridCtrl.scope.enableSortFirst && sortTypesIndex == 0) {
                                sortTypesIndex++;
                            }
                ...
                return {
                    post: function($scope, $elm, $attr, ctrls) {
                        ...
                        gridCtrl.showSelectAllTip = function() {
                            ...
                            var tb = config.tipsBeforeCheck,
                                ta = config.tipsAfterCheck;
                            var bodyInfo = gridScope.bodyInfo;
...
.directive('graceSimpleSelect', ['gridUtil', 'graceGridSelectDropDown', '$document', function(util, dropdown, doc) {
        ...
        return {
            restrict: 'EA',
            replace: true,
            template: '<span ng-click="showDropDown($event)" ng-class="{active:isActive}" class="custom-select grace-icon-caret-down" ><span>{{displayVal}}</span></span>',
            scope: {
                selectModel: "=",
                selectVal: "=",
                contentChange: '='
    ...
    .constant('graceInputConstant', {
        ...
        $scope.publishApi = function() {
            var api = {
                ...
                disable: function(isDisabled) {
                    $timeout(function() {
                        $scope.config.input ? ($scope.config.input.disabled = isDisabled) : ($scope.config.input = { disabled: isDisabled });
                    }, 0);
                },
                getValue: function() {
                    return $scope.getValue();
        ...
        return {
            ...
            link: function($scope, iElement, attrs, ctrls) {
                ...
                $scope.init = function() {
                    ...
                    if (!$scope.currentItem && !config.input.notInitFirst) {
...
.controller("summaryController", ["$scope", "summaryConfig", "summaryFactory", "$attrs", function($scope, summaryConfig, summaryFactory, $attrs) {
    ...
    $scope.overFlowItemChanged = function($event) {
        ...
        if ($scope.selectedOverFlowIndex == 0) {
        ...
        // 增加select 显示tooltip
        if ($scope.selectedOverFlow.desc) {
            $scope.showSelectTip = true;
            $scope.curSelectedItemTip = $scope.selectedOverFlow.desc;
        }
    }
...
.filter("summaryFilter", ["$filter", function($filter) {
    ...
    function formatNumber(input, param) {
        input = parseFloat(input);
        var tmp = param.split("_");
        var isShow = Number(tmp[3]);
        if (tmp[0] === 'percent' && Number(tmp[4])) {
            input = input * 100;
        }
        var res = $filter("number")(input, tmp[1] || 0);
        if (Number(tmp[2])) {
...
.directive("graceSummary", ["$timeout", "$window", "summaryConfig", function($timeout, $window, summaryConfig) {
    ...
    .filter("summaryCal", ["$filter", function($filter) {


        return function(input, param, direct) {
            var tmp = param.split('_'),
                isShow = Number(tmp[3]),
                input = Number(input);
            if (isShow && input > 0 && direct > 0) {
                return true;
            } else if (isShow && input < 0 && direct < 0) {
                return true;
            }
            return false;
...
filter("tableFilter", ['tableConfig', '$filter', function(config, $filter) {
    ...
    return function(input, fractionSize) {
        ...
        //如果是默认格式化类型
        if ((config.isFormat && config.formatType == config.NORMAL_TYPE) || (config.isFormat && !config.formatType)) {
...
angular.module("template/datepicker/day.html", []).run(["$templateCache", function($templateCache) {
    $templateCache.put("template/datepicker/day.html",
        ...
        "      <thead>\n" +
        "        <tr>\n" +
        "          <th class=\"th-arrow\"><button type=\"button\" class=\"grace-btn btn-default btn-sm pull-left\" ng-click=\"move(-1)\" tabindex=\"-1\" ng-class=\"{'disable': canNotMoveLeft}\"><i class=\"grace-icon-chevron-left\"></i></button></th>\n" +
        ...
        "          <th class=\"th-arrow\"><button type=\"button\" class=\"grace-btn btn-default btn-sm pull-right\" ng-click=\"move(1)\" tabindex=\"-1\" ng-class=\"{'disable': canNotMoveRight}\"><i class=\"grace-icon-chevron-right\"></i></button></th>\n" +
        "        </tr>\n" +
        "        <tr>\n" +
        ...
        "                      ng-click=\"selectWeek(weekNumbers[$index])\" ng-mouseover=\"hoverWeek(weekNumbers[$index])\" ng-mouseout=\"mouseoutWeek(weekNumbers[$index])\" ng-class=\"{active: weekNumbers[$index].selected, hovered:  weekNumbers[$index].hovered}\">{{ weekNumbers[$index].week }}</em>\n" +
        "          </td>\n" +
        "          <td ng-repeat=\"dt in row track by dt.date\" class=\"text-center\" role=\"gridcell\" id=\"{{dt.uid}}\" aria-disabled=\"{{!!dt.disabled}}\">\n" +
        "            <button type=\"button\" style=\"width:100%;\" class=\"grace-btn btn-default btn-sm {{dt.specialClass}}\"\n" +
        "                    ng-class=\"{'btn-info': dt.selected, active: dt.selected,'cell-muted': dt.secondary,weekend:dt.weekend, 'hovered':dt.hovered}\"\n" +
        "                    ng-click=\"select(dt.date)\" ng-disabled=\"dt.disabled\" tabindex=\"-1\" ng-mouseover=\"mouseover(dt.date)\"  ng-mouseout=\"mouseout(dt.date)\">\n" +
        "                <span ng-class=\"{'text-muted': dt.secondary, 'text-info': dt.current}\">{{dt.label}}</span></button>\n" +
        "          </td>\n" +
        "        </tr>\n" +
        "      </tbody>\n" +
        "    </table>\n" +
        "    <div ng-if=\"showTimeSet\" class=\"grace-time-set\">\n" +
        "        <input type=\"text\" class=\"grace-time-hour\" ng-keyup=\"hourValidate($event)\" ng-model=\"dt.hour\"><span>时</span><input type=\"text\" class=\"grace-time-minute\"  ng-keyup=\"minuteValidate($event)\" ng-model=\"dt.minute\"><span>分</span>\n" +
        "    </div>\n" +
        "</div>");
}]);
...
angular.module("template/pagination/pagination.html", []).run(["$templateCache", function($templateCache) {
    $templateCache.put("template/pagination/pagination.html",
        ...
        "  <li ng-if=\"directionLinks\" ng-class=\"{disabled: noPrevious()}\"><a href ng-click=\"selectPage(page - 1)\"><span class=\"pagination-next-prev\"><</span>{{getText('previous')}}</a></li>\n" +
        "  <li ng-repeat=\"page in pages track by $index\" ng-class=\"{active: page.active}\"><a href ng-click=\"selectPage(page.number)\">{{page.text}}</a></li>\n" +
        "  <li ng-if=\"directionLinks\" ng-class=\"{disabled: noNext()}\"><a href ng-click=\"selectPage(page + 1)\">{{getText('next')}}<span class=\"pagination-next-prev\">></span></a></li>\n" +
...
angular.module("template/grid/gridBody.html", []).run(["$templateCache", function($templateCache) {
    $templateCache.put("template/grid/gridBody.html",
        ...
        "            <div class=\"normal-scroller-inner\"></div>\n" +
        "    </div>\n" +
        "</div>\n" +
        "");
}]);
...
angular.module("template/select/select.html", []).run(["$templateCache", function($templateCache) {
    $templateCache.put("template/select/select.html",
        ...
        "        <div class=\"item-select\"  hidefocus=\"true\" dropdown-toggle ng-disabled=\"config.input.disabled\" ng-class=\"{'disable': config.input.disabled}\"  tooltip=\"{{validResult.text}}\" tooltip-trigger=\"click\" tooltip-placement=\"right\" >\n" +
...
angular.module("template/summary/summary.html", []).run(["$templateCache", function($templateCache) {
    $templateCache.put("template/summary/summary.html",
        ...
        "            ng-class=\"{disable:sum.disable,selected:selectedIndex == $index, 'has-detail':sum.detail&&sum.detail.length>0}\"\n" +
        "            ng-click=\"itemClick($index,$event)\">\n" +
        "            <div class=\"arrow-outer\" >\n" +
        "                <div class=\"arrow-inner\"></div>\n" +
        "            </div>\n" +
        "            <h6 class=\"summary-title\">\n" +
        "                <label ng-if=\"(!!sum.titlePos)&&(sum.titlePos!='right')\">{{sum.title}}</label>\n" +
        ...
        "                <div style=\"height: 100%;display: inline-block;\" class=\"unit\" ng-if=\"sum.mainValue.isCurrency\">¥</div>\n" +
        ...
        "            </div> \n" +
        "\n" +
        "            <div class=\"summary-detail\"  ng-if=\"sum.detail && sum.detail.length>0\">\n" +
        "                <ul>\n" +
        "                    <li ng-repeat=\"detailItem in sum.detail\" class=\"clearfix\">\n" +
        "                        <label ng-bind=\"detailItem.name\"\n" +
        "                           class=\"detail-label\"></label>\n" +
        "                        <span ng-bind=\"detailItem.value | summaryFilter:detailItem.format\"\n" +
        "                           class=\"detail-value\" ng-class=\"{'up-ar': (detailItem.value|summaryCal:detailItem.format:1), 'down-ar':(detailItem.value|summaryCal:detailItem.format:-1)}\"></span>\n" +
        "                    </li>\n" +
        "                </ul>\n" +
        "            </div> \n" +
        "\n" +
        ...
        "                   ng-class=\"{'up-ar': (sum.footLeftValue.value|summaryCal:sum.footLeftValue.format:1), 'down-ar':(sum.footLeftValue.value|summaryCal:sum.footLeftValue.format:-1)}\"\n" +
        "\n" +
        ...
        "                   ng-class=\"{'up-ar': (sum.footRightValue.value|summaryCal:sum.footRightValue.format:1), 'down-ar':(sum.footRightValue.value|summaryCal:sum.footRightValue.format:-1)}\"\n" +
        "\n" +
